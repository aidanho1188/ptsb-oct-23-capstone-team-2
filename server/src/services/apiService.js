...
async function fetchData(endpoint, select, filter) {
  ...
  const response = await axios.get(
    `https://sb2api.servicechannel.com/v3/odata/${endpoint}?$select=${select}&$filter=${filter}`,
    {
      headers: {
...
async function fetchWorkOrder(
  workOrderId,
  select = "Id,Location.Id,Trade,Status"
) {
  console.log(`Working on fetching data from Service Channel...`);
  try {
    const response = await axios.get(
      `https://sb2api.servicechannel.com/v3/odata/workorders(${workOrderId})?$select=${select}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${await getAccessToken(tokenType)}`,
...
async function fetchWorkActivities(
  workOrderId,
  select = "TimeIn,TimeOut,User,ResolutionCode,TechsCount"
) {
  console.log("Working on fetching data from Service Channel...");
  try {
    // Fetching work activities
    const response = await axios.get(
      `https://sb2api.servicechannel.com/v3/odata/workorders(${workOrderId})/workactivities?$select=${select}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${await getAccessToken(tokenType)}`,
...
async function fetchLocation(locationId) {
  console.log(`Working on fetching data from Service Channel...`);
  const response = await axios.get(
    `https://sb2api.servicechannel.com/v3/locations/${locationId}`,
    {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${await getAccessToken(tokenType)}`,
...
async function sendStatusUpdateRequest(
  workOrderId,
  primary,
  extended,
  note,
